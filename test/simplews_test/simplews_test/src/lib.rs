...
fn init(our: Address) {
    ...
    loop {
        match handle_message(&our) {
            Ok(()) => {},
            Err(e) => {
                print_to_terminal(0, format!("counter_test: error: {e:?}").as_str());
